...
---Crash-replay slice request. Server → client, sent on the player's next
---quiescent moment (login or lobby return) for each incident the player
---is expected to report on. The client looks up local replay data
---matching `gameKey` (typically a pending_crashes/ spool entry) and ships
---it back via crashSlice. See docs/CRASH_REPLAY_PLAN.md "Wire shape".
---@param incidentId string
---@param gameKey table {roomNumber, gameId?, startTs}
---@param reason string the trigger reason recorded server-side
---@return {messageType: table, messageText: ServerMessage}
function ServerProtocol.crashSliceRequest(incidentId, gameKey, reason)
  return {
    messageType = msgTypes.jsonMessage,
    messageText = {
      sender  = "server",
      type    = "crashSliceRequest",
      content = {
