...
function TimeAttackMenu:loadUserInterface()
  ...
  local difficultyCarousel = self:createDifficultyCarousel(player, self.ui.grid.unitSize - self.ui.grid.unitMargin * 2 - self.ui.difficultySelection.height, LevelPresets.getClassic)
  ...
  styleSelector.onValueChange = function(boolSelector, value)
    ...
    elseif value == false and player.settings.style ~= GameModes.Styles.CLASSIC then
      -- Set levelData for classic style - this will trigger levelDataChanged signal which updates UI
...
function TimeAttackMenu:initializeFromLocalPlayerSettings(player)
